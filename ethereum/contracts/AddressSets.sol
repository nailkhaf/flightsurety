...
library AddressSets {
    ...
    function addAddress(AddressSet storage addressSet, address addr) internal {
        require(!addressSet.addresses[addr], "Address already exist AddressSet");
        addressSet.addresses[addr] = true;
        addressSet.size = addressSet.size.add(1);
    }

    function deleteAddress(AddressSet storage addressSet, address addr) internal {
    ...
    function containsAddress(AddressSet storage addressSet, address addr) internal view returns (bool) {
        return addressSet.addresses[addr];
    }

    function countAdresses(AddressSet storage addressSet) internal view returns (uint256) {
